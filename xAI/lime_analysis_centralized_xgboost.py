"""
Análise LIME para XGBoost Centralizado.
...
Este script espera o modelo centralizado em:
  centralized_training/models/xgboost/centralized_model_best.joblib
"""
...
flwr_xgb_path = project_root / 'flwr-xgboost'
if str(flwr_xgb_path) not in sys.path and flwr_xgb_path.exists():
    sys.path.insert(0, str(flwr_xgb_path))
...
def build_predict_fn_xgb(model, feature_names):
    """Cria função predict_proba compatível com XGBClassifier ou xgb.Booster."""
    ...
    try:
        ...
        if isinstance(model, xgb.Booster) or 'booster' in str(type(model)).lower():
            def predict_proba(X):
                Xnp = np.asarray(X)
                # DMatrix precisa dos feature_names para não reclamar de validação
                ...
                if preds.ndim == 1:
                    ...
                    probs = preds.reshape((preds.shape[0], -1))
                return probs
            return predict_proba
    except Exception:
        pass

    # Último recurso: tenta predict genérico
    ...
    raise RuntimeError("Não foi possível criar predict_proba para este modelo XGBoost.")


def explain_instances_xgb(args):
    ...
    model_path = project_root / 'centralized_training' / 'models' / 'xgboost' / 'centralized_model_best.joblib'
    if not model_path.exists():
        raise FileNotFoundError(f"Model not found: {model_path}")

    model_data = load_model_from_joblib(model_path)
    # Extrai modelo do dict (pode estar em 'best_model' ou direto)
    ...
    # Preprocessa dados de treino para obter feature names (usado no DMatrix)
    ...
    predict_proba = build_predict_fn_xgb(model, feature_names_for_model)
    ...
    X_pool, y_pool, feature_names_pool = utils_load_shap_samples(
        project_root / 'datasets' / 'rain_australia' / 'weatherAUS_cleaned.csv',
        project_root / 'datasets' / 'shap_sample_indices.csv',
        preprocess_fn=_preprocess_xgb
    )

    feature_names = feature_names_pool
    ...
    out_dir = project_root / 'xAI' / 'lime_results_centralized' / 'xgboost'
    out_dir.mkdir(parents=True, exist_ok=True)

    per_instance_records = []

    print(f"Gerando explicações LIME para {len(chosen_local)} instâncias (XGBoost)...")
    ...
    plt.title('LIME - Importância Global (mean |coef|) - XGBoost - Top {}'.format(TOP_K))
    plt.tight_layout()
    plt.savefig(out_dir / f'lime_xgb_global_mean_abs_weight_top{TOP_K}.png')
    plt.close()
    ...
    for cls in [0, 1]:
        ...
        plt.title(f'LIME XGBoost - Importância Classe {cls} (mean |coef|) - Top {TOP_K_C}')
        plt.tight_layout()
        plt.savefig(out_dir / f'lime_xgb_class_{cls}_mean_abs_weight_top{TOP_K_C}.png')
        plt.close()
    ...
    plt.title('Distribuição dos coeficientes LIME (signed) por feature (Top {}) - XGBoost'.format(TOP_K))
    plt.tight_layout()
    plt.savefig(out_dir / f'lime_xgb_feature_weight_distribution_top{TOP_K}.png')
    plt.close()
    ...
    plt.title(f'Heatmap das contribuições LIME (signed) - XGBoost - Top {len(common_cols)} features')
    plt.tight_layout()
    plt.savefig(out_dir / f'lime_xgb_heatmap_top{len(common_cols)}_rows{sampled.shape[0]}.png')
    plt.close()
    ...
    for cls, inst_idx in rep_instances:
        ...
        plt.title(f'LIME - Waterfall-like contributions (idx={inst_idx}, true_label={cls}) - XGBoost')
        plt.tight_layout()
        plt.savefig(out_dir / f'lime_xgb_waterfall_idx{inst_idx}_label{cls}.png')
        plt.close()

    summary_lines = []
    summary_lines.append('# Resumo LIME - Centralizado (XGBoost)\n')
